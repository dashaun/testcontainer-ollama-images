...
public class ImageGeneratorCommand {
    ...
    public void generate(
            ...
            @Option(longName = "ollamaVersion", required = false,
                    description = "ollama/ollama tag to build from; defaults to latest") String ollamaVersion) throws Exception {
        ...
        String baseImage = "ollama/ollama:" + (ollamaVersion == null || ollamaVersion.isBlank() ? "latest" : ollamaVersion);
        ...
        try (OllamaContainer ollama = new OllamaContainer(baseImage)) {
            ...
            // Without an explicit version the base tag is "latest", which would collide with the
            // latest tag we push. Ask the running engine what it actually is and tag with that.
            ...
            for (String model : modelList) {
                ...
                // execInContainer never throws on a non-zero exit, so an unknown model would
                // otherwise be committed as a silently empty image.
                if (result.getExitCode() != 0) {
                    throw new IllegalStateException("ollama pull " + model + " failed (exit "
                            + result.getExitCode() + "): " + lastLine(result.getStderr()));
                }
            }
    ...
    // `ollama pull` draws a progress spinner, so its stderr is mostly ANSI escapes; the real
    ...
    private String resolveOllamaVersion(OllamaContainer ollama) throws Exception {
        ...
        if (result.getExitCode() != 0 || !matcher.find()) {
            throw new IllegalStateException("Could not resolve the ollama version from the container "
                    + "(exit " + result.getExitCode() + "): " + output
                    + " — pass --ollamaVersion explicitly");
        }
